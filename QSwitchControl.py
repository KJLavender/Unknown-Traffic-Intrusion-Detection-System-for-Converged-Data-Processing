...
class SwitchControl(QCheckBox):
    def __init__(self, parent=None, bg_color="#777777", circle_color="#DDD", active_color="#aa00ff",
        ...
        self.__circle = SwitchCircle(self, (3, self.width() - self.widthM), self.circle_color, self.animation_curve,
        ...
        if checked:
            self.__circle.move(self.width() - self.widthM, 3)
    ...
    circleBackgroundColor = pyqtProperty(
        str, get_circle_color, set_circle_color)
    ...
    def start_animation(self, checked):
        ...
        if checked:
            self.animation.setEndValue(
                QPoint(self.width() - self.widthM, self.__circle.y()))
            self.setChecked(True)
        if not checked:
